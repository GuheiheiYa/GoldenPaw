...
**状态**: 已完成（数据层待完善）  **实现时间**: 2026-05-28 00:00:00  **最后更新**: 2026-06-04 11:26:45
...
**已知问题**:
- [ISS-021] 待确认页面全部使用硬编码 mock 数据（3条 reactive 写死），没有接入任何 Store
...
**已知问题**:
- [ISS-021] 数据来源为硬编码 `reactive([])`，刷新即丢失

---

## [F-064] 待确认 - 确认按钮 ← [R-064]

**状态**: 已完成（数据层待完善）  **实现时间**: 2026-05-29 00:00:00  **最后更新**: 2026-06-04 11:26:45
...
**已知问题**:
- [ISS-021] 虽然确认逻辑正确，但操作的是内存中的假数据，无实际意义
...
**已知问题**:
- [ISS-021] 忽略只从内存数组中 splice，刷新页面后数据恢复
...
**实现方式**:
- 来源开关切换（招商银行/支付宝/微信支付）

**关联文件**:
- `src/pages/pending/pending.vue`

**已知问题**:
- [ISS-021] 来源列表为硬编码 reactive，开关状态不持久化
- [ISS-026] 实际无真实短信/通知监听能力，无法产生待确认数据
...
**状态**: 已完成  **实现时间**: 2026-05-28 00:00:00  **最后更新**: 2026-06-04 11:26:45
...
**已知问题**:
- [ISS-024] 云同步时间显示假数据（"上次同步: 5分钟前"写死）
- [ISS-025] 导出数据菜单描述写 "CSV / Excel 格式"，但 Excel 导出未实现

---

## [F-083] 我的/设置 - 数据导入 ← [R-083]

**状态**: 已完成  **实现时间**: 2026-06-02 10:33:56

**实现方式**:
- H5 环境通过 `<input type="file">` 选择 CSV 文件，FileReader 读取内容
- 智能格式识别：根据 CSV 标题行自动识别 GoldenPaw / 微信 / 支付宝格式
- `csvImport.ts` 统一解析逻辑：
  - GoldenPaw 格式：日期,时间,类型,金额,分类,账户,备注
  - 微信账单：交易时间,交易类型,交易对方,商品,收/支,金额(元)...
  - 支付宝账单：交易创建时间,类型,交易对方,商品名称,收/支,金额...
- 自动创建缺失的分类和默认账户
- 导入后显示结果弹窗（成功/失败条数）
...
**注意事项**:
- 仅在 H5 环境可用（依赖 FileReader API）
- 导入金额按元处理（自动 ×100 转为分存储）
...
**实现方式**:
- `recurringStore`（Pinia + persist）管理定期交易规则列表
- 支持 4 种周期：每天/每周/每月/每年
- 规则字段：名称、金额、类型、分类、账户、周期、每月几号、下次执行日期、启用状态
- `App.vue` 启动时调用 `executeRecurringTransactions()`，遍历到期规则自动创建交易并更新账户余额
- 执行后自动推进 `nextDate` 到下一个周期
- settings/index.vue 提供完整的规则管理：添加/编辑/删除/启用禁用
...
**注意事项**:
- 仅 APP 启动时检查执行，后台运行时不会自动触发（需配合推送通知或定时任务）
- 月末日期处理：如设置 31 日，2 月自动取 28/29 日
...
- 同步策略：
  ...
  - **恢复**：输入其他设备的 `syncKey`，拉取云端数据覆盖本地
- `syncKey`：存储在 `appStore.syncKey`，首次同步时自动生成（`Date.now()` + 随机字符串）
- `isSupabaseConfigured()` 检查 `.env` 中是否配置了 `VITE_SUPABASE_URL` 和 `VITE_SUPABASE_ANON_KEY`
- 未配置时显示提示卡片，引导用户配置环境变量

**关联文件**:
- `src/utils/supabase.ts` — Supabase 客户端配置
- `src/utils/sync.ts` — 上传/下载/合并逻辑
- `src/pages/settings/index.vue` — 同步 UI（同步码显示/复制、上传/下载按钮、恢复输入框）
- `src/stores/app.ts` — 新增 `syncKey` + `setSyncKey()`

**使用方式**:
1. 在项目根目录创建 `.env`，填入 Supabase URL 和 Anon Key（详见 `.env.example`）
2. 重新编译项目
3. 进入「我的 → 设置 → 云同步」
4. 点击「上传到云端」生成同步码
5. 在其他设备输入同步码点击「恢复数据」即可同步

**注意事项**:
- Supabase 免费额度：500MB 数据库、50K MAU、1GB 存储，对记账App完全够用
- 7 天不活跃项目自动暂停，可通过定时 ping 保持活跃
- 同步为全量覆盖模式，暂不支持增量/冲突合并
